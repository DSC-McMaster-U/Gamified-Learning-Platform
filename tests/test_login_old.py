# Old test_login.py for reference, delete later if not needed
...
class LoginTestCase(unittest.TestCase):
    ...
    def TestLoginHTMLHeader(self):
        # Tests if certain HTML elements are present on the page
        response=self.client.get('/login')
        self.assertEqual(response.status_code, 200) 

        # Tests for presence of header
        self.assertIn(b'<h1>Sign in to Your Account</h1>', response.data)

    def TestLoginHTMLForm(self):
        response=self.client.get('/login')
        self.assertEqual(response.status_code, 200) 
    ...
    def TestLoginInvalidHTMLForm(self):
        response=self.client.get('/login')
        self.assertEqual(response.status_code, 200) 
